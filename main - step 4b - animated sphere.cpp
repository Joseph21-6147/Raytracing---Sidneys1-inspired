...
public:
    ...
    Shape( vf3d _origin, olc::Pixel _fill, float _reflectivity = 0.0f ) : origin( _origin ), fill( _fill ), reflectivity( _reflectivity ) {}
...
public:
    ...
    Sphere( vf3d _origin, olc::Pixel _fill, float _radius, float _reflectivity = 0.0f ) : Shape( _origin, _fill, _reflectivity ), radius(_radius ) {}
...
public:
    bool OnUserCreate() override {
        ...
        shapes.emplace_back( std::make_unique<Sphere>( vf3d(    0,   0,  200 ), olc::GREY,  100.0f, 0.9f ));
        // add some additional Spheres at different positions
        shapes.emplace_back( std::make_unique<Sphere>( vf3d( -150, +75, +300 ), olc::RED,   100.0f, 0.5f ));
